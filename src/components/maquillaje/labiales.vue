<template>   
  <div class="labiales-page">
    <!-- Header de la marca -->
    <section class="brand-header">
      <div class="container">
        <div class="brand-intro">
          <div class="brand-logo">
            <img src="@/assets/maquillaje/labiales.webp" alt="Anua" />
          </div>
      ...
      <div class="container">
        ...
        <div class="category-section">
          ...
          <div class="products-grid">
            <div 
              v-for="product in (labialesProducts || [])" 
              :key="product.id || product.name"
              class="product-card"
            >
              <div class="product-image" :class="`image-${product.imageSize}`" @click="openImageModal(selectedTono[product.id]?.imagen || product.src)">
                <img v-if="selectedTono[product.id]?.imagen || product.src" :src="selectedTono[product.id]?.imagen || product.src" :alt="product.name" style="cursor: pointer;" />
              ...
              <div class="product-info">
                ...
                <!-- Selector de tonos -->
                <div v-if="product.tonos?.length" class="tonos-selector">
                  <p class="tonos-label">Tonos:</p>
                  <div class="tonos-buttons">
                    <button 
                      v-for="tono in product.tonos" 
                      :key="tono.nombre"
                      class="tono-btn"
                      :style="{ backgroundColor: tono.color }"
                      :class="{ 'tono-selected': selectedTono[product.id]?.nombre === tono.nombre }"
                      @click="selectTono(product.id, tono)"
                      :title="tono.nombre"
                    ></button>
                  </div>
                  <p v-if="selectedTono[product.id]" class="tono-seleccionado">{{ selectedTono[product.id].nombre }}</p>
                </div>
                
                <div class="product-price">{{ product.price }}</div>
                <button 
                  class="add-to-cart-btn" 
                  @click="addToCart(product, selectedTono[product.id])"
                  :disabled="product.tonos?.length > 0 && !selectedTono[product.id]"
                >
                  {{ product.tonos?.length > 0 && !selectedTono[product.id] ? 'Selecciona un tono' : 'Agregar al Carrito' }}
                </button>
              </div>
            </div>
          </div>
...
export default {
  ...
  setup() {
    ...
    const selectedTono = ref({});
    let toastTimer = null;
    // Productos de Skin1004 organizados
       const products = [
      // Gege bear
       {
      id: "gege-bear-lipstick-01",
      name: "Tinta Labial Gege Bear",
      tamanio: "4 gr",
      price: "4.25",
      src: "/images/labiales/gegebear.jpg",
      tonos: [
        { nombre: "Tono 1", color: "#FF3E41", imagen: "/images/labiales/gegebear_1.jpg" },
        { nombre: "Tono 2", color: "#90323D", imagen: "/images/labiales/gegebear_2.jpg" },
      ],
      category: "labiales"
    },
    ...
    const labialesProducts = computed(() => 
      (products.filter(product => product.category === 'labiales') || []).filter(p => p && p.id)
    );
  ...
  const selectTono = (productId, tono) => {
    selectedTono.value[productId] = tono;
  };

  const addToCart = (product, tono) => {
      try {
        // Validar que si el producto tiene tonos, se haya seleccionado uno
        if (product.tonos?.length > 0 && !tono) {
        ...
        if (cartStore && cartStore.addItem) {
          // Crear un objeto de carrito que incluya el tono seleccionado
          const itemToAdd = {
            ...product,
          ...
          const tonoText = tono ? ` - ${tono.nombre}` : "";
          toastMessage.value = `🛒 ${product.name} ${product.tamanio ? `(${product.tamanio})` : ""}${tonoText} agregado al carrito`;
...
/* Estilos para selector de tonos */
.tonos-selector {
  margin-bottom: 1rem;
  padding: 0.8rem 0;
